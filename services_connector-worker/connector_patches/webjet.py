...
from .browser import find_chrome, stealth_popen_kwargs, _launched_procs, proxy_chrome_args, bandwidth_saving_args, disable_background_networking_args, apply_cdp_url_blocking, get_httpx_proxy_url

logger = logging.getLogger(__name__)

_CDP_PORT = 9482
_USER_DATA = os.path.join(
    os.environ.get("TEMP", os.environ.get("TMPDIR", "/tmp")), ".webjet_chrome_data"
)
_CALENDAR_OW_URL = "https://services.webjet.com.au/api/flights/calendar/oneway"
_CALENDAR_HEADERS = {
    "User-Agent": (
    ...
    "Origin": "https://www.webjet.com.au",
    "Referer": "https://www.webjet.com.au/",
}
...
class WebjetConnectorClient:
        ...
        for attempt in range(1):
            try:
                ...
                if offers:
                    ...
                    logger.info(
                        "WBJT %s→%s: %d offers in %.1fs (matrix)",
                        req.origin, req.destination, len(offers), elapsed,
                    )
                    return self._build_response(req, offers)
            except Exception as e:
                logger.warning("WBJT attempt %d failed: %s", attempt, e)
                if attempt == 0:
                    await _reset_profile()

        # Matrix endpoint is often Cloudflare-blocked. Fallback to the public
        # calendar endpoint to return real priced offers instead of zero results.
        cal_offers = await self._search_calendar_fallback(req, dt)
        if cal_offers:
            elapsed = time.monotonic() - t0
            logger.info(
                "WBJT %s→%s: %d offers in %.1fs (calendar fallback)",
                req.origin, req.destination, len(cal_offers), elapsed,
            )
            return self._build_response(req, cal_offers)

        return self._empty(req)

    def _build_response(
        self, req: FlightSearchRequest, offers: list[FlightOffer], default_currency: str = "AUD",
    ) -> FlightSearchResponse:
        ...
        return FlightSearchResponse(
            ...
            currency=offers[0].currency if offers else default_currency,
            offers=offers,
            total_results=len(offers),
        )

    async def _is_challenge_page(self, page) -> bool:
        try:
    ...
    async def _search_calendar_fallback(
        self, req: FlightSearchRequest, target_dt: datetime,
    ) -> list[FlightOffer]:
        params = {
            "departureAirportCode": req.origin.upper(),
            "destinationAirportCode": req.destination.upper(),
            "travelClass": {
                "M": "Economy",
                "W": "PremiumEconomy",
                "C": "Business",
            ...
            "departYear": str(target_dt.year),
            "departMonth": str(target_dt.month),
        }

        try:
            async with httpx.AsyncClient(
                timeout=min(self.timeout, 18.0),
                headers=_CALENDAR_HEADERS,
                follow_redirects=True,
                proxy=get_httpx_proxy_url(),
            ) as client:
                resp = await client.get(_CALENDAR_OW_URL, params=params)
        except Exception as e:
            logger.debug("WBJT calendar fallback request failed: %s", e)
            return []

        if resp.status_code != 200:
            logger.debug("WBJT calendar fallback status=%s", resp.status_code)
            return []
        ...
        for row in rows:
            if not isinstance(row, dict):
                continue
            if not row.get("hasScheduledFlights"):
                continue
            min_price = row.get("minPrice")
            if not isinstance(min_price, (int, float)) or float(min_price) <= 0:
                continue

            dep_dt = _parse_dt(row.get("departDate"))
            if dep_dt.year < 2005:
                continue

            dedup = f"{dep_dt:%Y%m%d}_{float(min_price):.2f}"
            if dedup in seen:
                continue
            seen.add(dedup)
            ...
            fid = hashlib.md5(f"wbjt_cal_{req.origin}_{req.destination}_{dedup}".encode()).hexdigest()[:12]
            offer = FlightOffer(
                id=f"wbjt_{fid}",
                price=round(float(min_price), 2),
                currency="AUD",
                price_formatted=f"{float(min_price):.2f} AUD",
                outbound=route,
                inbound=None,
                airlines=["Webjet"],
                owner_airline="WJ",
                booking_url=(
                    ...
                    f"{req.origin.lower()}/{req.destination.lower()}/{dep_dt:%Y%m%d}/"
                ),
                is_locked=False,
                source="webjet_ota",
                source_tier="free",
            ...
            day_delta = abs((dep_dt.date() - target_dt.date()).days)
            ranked.append((day_delta, offer))

        if not ranked:
            return []

        exact = [o for d, o in ranked if d == 0]
        if exact:
            exact.sort(key=lambda o: o.price)
            return exact[:8]

        ranked.sort(key=lambda p: (p[0], p[1].price))
        return [o for _, o in ranked[:8]]
    ...
    async def _do_search(
        ...
        try:
            # Warm up first-party domain before matrix search. This often avoids
            # getting stuck on Cloudflare's interstitial at services.webjet.com.au.
            try:
                await page.goto("https://www.webjet.com.au/", wait_until="domcontentloaded", timeout=30000)
                await asyncio.sleep(3)
            except Exception as e:
                logger.debug("WBJT: warm-up navigation failed: %s", e)

            logger.info("WBJT: navigating to %s", search_url)
            await page.goto(search_url, wait_until="domcontentloaded", timeout=30000)

            # If Cloudflare challenge appears, try one short recovery cycle.
            try:
                page_title = (await page.title()).lower()
                if "just a moment" in page_title:
                    logger.warning("WBJT: Cloudflare challenge detected, retrying matrix navigation")
            ...
            if await self._is_challenge_page(page):
                logger.warning("WBJT: blocked by Cloudflare challenge on matrix endpoint")
                return []
            ...
            while time.monotonic() < deadline:
                ...
                if matrix_blocked and not captured_data:
                    logger.warning("WBJT: matrix endpoint returned blocked status")
                    return []
