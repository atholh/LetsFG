...
class FlyadealConnectorClient:
    ...
    def _build_offers(
        ...
        for card in cards:
            ...
            _f3_cabin = {"M": "economy", "W": "premium_economy", "C": "business", "F": "first"}.get(req.cabin_class or "M", "economy")
            outbound = _build_route(
                req.origin, req.destination, card["departure_date"], _f3_cabin
            )
            inbound = None
            if card.get("return_date"):
                inbound = _build_route(
                    req.destination, req.origin, card["return_date"], _f3_cabin
                )
