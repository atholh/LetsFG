"""
Jeju Air Playwright connector — browser automation to bypass WAF.

Jeju Air (IATA: 7C) is a South Korean LCC based at Jeju International Airport.
Website: www.jejuair.net — English version at /en.

Strategy:
1. Navigate to jejuair.net/en homepage
...
_LOCALES = ["en-US", "en-GB", "en-KR", "ko-KR"]
_TIMEZONES = [
...
async def _get_browser():
    ...
    async with lock:
        ...
        logger.info("JejuAir: Playwright browser launched (headed Chrome)")
        return _browser


class JejuAirConnectorClient:
    """Jeju Air Playwright connector — homepage form search + API interception."""
    ...
    async def search_flights(self, req: FlightSearchRequest) -> FlightSearchResponse:
        ...
        try:
            ...
            logger.info("JejuAir: loading homepage for %s→%s", req.origin, req.destination)
            await page.goto("https://www.jejuair.net/en/main/base/main.do",
            ...
            if not ok:
                logger.warning("JejuAir: origin fill failed")
                return self._empty(req)
            await asyncio.sleep(0.5)

            ok = await self._fill_airport_field(page, req.destination, 1)
            if not ok:
                logger.warning("JejuAir: destination fill failed")
                return self._empty(req)
            await asyncio.sleep(0.5)

            ok = await self._fill_date(page, req)
            if not ok:
                logger.warning("JejuAir: date fill failed")
                return self._empty(req)
            ...
            try:
                ...
                logger.warning("JejuAir: timed out waiting for API response")
            ...
            logger.error("JejuAir Playwright error: %s", e)
            return self._empty(req)
        finally:
            await context.close()

    async def _dismiss_cookies(self, page) -> None:
        # JejuAir may show promotional popup or language selector — close those
        try:
    ...
    async def _fill_airport_field(self, page, iata: str, index: int) -> bool:
        origin_labels = ["From", "Departure", "Origin", "출발"]
        dest_labels = ["To", "Arrival", "Destination", "도착"]
        ...
        try:
            ...
            logger.debug("JejuAir: field %d error: %s", index, e)
        try:
    ...
    async def _fill_date(self, page, req: FlightSearchRequest) -> bool:
        ...
        try:
            for name in ["Departure", "Depart", "Date", "출발일"]:
            ...
            logger.warning("JejuAir: date error: %s", e)
            return False

    async def _click_search(self, page) -> None:
        for label in ["Search", "SEARCH", "Search Flights", "Find Flights", "항공편 검색"]:
            try:
                btn = page.get_by_role("button", name=re.compile(rf"^{re.escape(label)}$", re.IGNORECASE))
                if await btn.count() > 0:
    ...
    def _parse_single_flight(self, flight: dict, req: FlightSearchRequest, booking_url: str) -> Optional[FlightOffer]:
        ...
        return FlightOffer(
            id=f"7c_{hashlib.md5(str(flight_key).encode()).hexdigest()[:12]}",
            price=round(best_price, 2), currency=req.currency,
            price_formatted=f"{best_price:.2f} {req.currency}",
            outbound=route, inbound=None, airlines=["Jeju Air"], owner_airline="7C",
            booking_url=booking_url, is_locked=False, source="jejuair_direct", source_tier="free",
        )
    ...
    def _build_segment(self, seg: dict, default_origin: str, default_dest: str) -> FlightSegment:
        ...
        carrier = seg.get("carrierCode") or seg.get("carrier") or seg.get("airline") or "7C"
        return FlightSegment(
            airline=carrier, airline_name="Jeju Air", flight_no=flight_no,
    ...
    def _build_response(self, offers: list[FlightOffer], req: FlightSearchRequest, elapsed: float) -> FlightSearchResponse:
        ...
        logger.info("JejuAir %s→%s returned %d offers in %.1fs", req.origin, req.destination, len(offers), elapsed)
        h = hashlib.md5(f"jejuair{req.origin}{req.destination}{req.date_from}".encode()).hexdigest()[:12]
    ...
    def _build_booking_url(req: FlightSearchRequest) -> str:
        ...
        return (
            f"https://www.jejuair.net/en/booking/search?origin={req.origin}"
