"""
Batik Air Playwright connector — browser automation to bypass WAF.

Batik Air (IATA: ID) is an Indonesian airline (Lion Air Group).
Website: www.batikair.com → redirects to batikair.com.my.

Strategy:
1. Navigate to batikair.com homepage
...
_LOCALES = ["en-US", "en-GB", "en-MY", "en-ID", "id-ID"]
_TIMEZONES = [
    "Asia/Jakarta", "Asia/Kuala_Lumpur", "Asia/Singapore",
    "Asia/Bangkok", "Asia/Makassar",
]
...
async def _get_browser():
    ...
    async with lock:
        ...
        logger.info("BatikAir: Playwright browser launched (headed Chrome)")
        return _browser


class BatikAirConnectorClient:
    """Batik Air Playwright connector — homepage form search + API interception."""
    ...
    async def search_flights(self, req: FlightSearchRequest) -> FlightSearchResponse:
        ...
        try:
            ...
            await page.goto("https://www.batikair.com/",
            ...
            try:
                ...
                logger.warning("BatikAir: timed out waiting for API response")
            ...
            logger.error("BatikAir Playwright error: %s", e)
            return self._empty(req)
        finally:
            await context.close()
    ...
    async def _set_one_way(self, page) -> None:
        for label in ["One Way", "One-way", "One way", "Sekali Jalan"]:
            try:
    ...
    async def _fill_airport_field(self, page, iata: str, index: int) -> bool:
        origin_labels = ["From", "Origin", "Departure", "Asal"]
        dest_labels = ["To", "Destination", "Arrival", "Tujuan"]
        ...
        try:
            ...
            logger.debug("BatikAir: field %d error: %s", index, e)
        try:
    ...
    async def _fill_date(self, page, req: FlightSearchRequest) -> bool:
        ...
        try:
            for name in ["Departure", "Depart", "Date", "Tanggal"]:
    ...
    async def _click_search(self, page) -> None:
        for label in ["Search", "SEARCH", "Search Flights", "Find Flights", "Cari Penerbangan"]:
            try:
                btn = page.get_by_role("button", name=re.compile(rf"^{re.escape(label)}$", re.IGNORECASE))
                if await btn.count() > 0:
    ...
    def _parse_single_flight(self, flight: dict, req: FlightSearchRequest, booking_url: str) -> Optional[FlightOffer]:
        ...
        return FlightOffer(
            id=f"id_{hashlib.md5(str(flight_key).encode()).hexdigest()[:12]}",
            price=round(best_price, 2), currency=req.currency,
            price_formatted=f"{best_price:.2f} {req.currency}",
            outbound=route, inbound=None, airlines=["Batik Air"], owner_airline="ID",
            booking_url=booking_url, is_locked=False, source="batikair_direct", source_tier="free",
        )
    ...
    def _build_segment(self, seg: dict, default_origin: str, default_dest: str) -> FlightSegment:
        ...
        carrier = seg.get("carrierCode") or seg.get("carrier") or seg.get("airline") or "ID"
        return FlightSegment(
            airline=carrier, airline_name="Batik Air", flight_no=flight_no,
    ...
    def _build_booking_url(req: FlightSearchRequest) -> str:
        ...
        return (
            f"https://www.batikair.com/booking/search?origin={req.origin}"
