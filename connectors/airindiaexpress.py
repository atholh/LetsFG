"""
Air India Express direct connector — uses Playwright to bypass WAF protection.

Air India Express (IATA: IX) is an Indian low-cost carrier (subsidiary of Air India).
Website: www.airindiaexpress.com — may redirect through tracking (doubleclick).

Strategy:
1. Navigate to airindiaexpress.com homepage
...
- Site uses redirects through doubleclick for tracking
- Modern booking form on homepage
- May need extra wait time for SPA hydration
"""
...
async def _get_browser():
    ...
    async with lock:
        ...
        logger.info("AirIndiaExpress: Playwright browser launched (headed Chrome)")
        return _browser


class AirIndiaExpressConnectorClient:
    """Air India Express Playwright connector — form-fill + API interception."""
    ...
    async def search_flights(self, req: FlightSearchRequest) -> FlightSearchResponse:
        ...
        try:
            ...
            async def on_response(response):
                try:
                    ...
                    if response.status == 200 and (
                        ...
                        or "offers" in url
                        or "low-fare" in url
                        or "ndc" in url and ("search" in url or "offer" in url)
            ...
            logger.info("AIX: loading homepage for %s→%s", req.origin, req.destination)
            await page.goto(
            ...
            if not ok:
                logger.warning("AIX: origin fill failed")
                return self._empty(req)
            await asyncio.sleep(0.5)

            ok = await self._fill_airport_field(page, "To", req.destination, 1)
            if not ok:
                logger.warning("AIX: destination fill failed")
                return self._empty(req)
            await asyncio.sleep(0.5)

            ok = await self._fill_date(page, req)
            if not ok:
                logger.warning("AIX: date fill failed")
                return self._empty(req)
            ...
            try:
                ...
                logger.warning("AIX: timed out waiting for API response")
            ...
            logger.error("AIX Playwright error: %s", e)
            return self._empty(req)
        finally:
            await context.close()
    ...
    async def _dismiss_cookies(self, page) -> None:
        for label in [
            "Accept All", "Accept Cookies", "Accept all cookies", "Accept",
            "I agree", "Got it", "OK", "Close", "Allow All", "Dismiss",
        ]:
            try:
        ...
        try:
            await page.evaluate("""() => {
                document.querySelectorAll(
                    ...
                    '[class*="modal-overlay"], [class*="privacy"], [id*="privacy"]'
    ...
    async def _fill_airport_field(self, page, label: str, iata: str, index: int) -> bool:
        try:
            ...
            logger.debug("AIX: %s field error: %s", label, e)
        # fallback by index
        try:
    ...
    async def _fill_date(self, page, req: FlightSearchRequest) -> bool:
        ...
        try:
            for name in ["Departure", "Depart", "Date", "When", "Travel Date", "Journey Date"]:
            ...
            logger.warning("AIX: date error: %s", e)
            return False

    async def _click_search(self, page) -> None:
        for label in ["Search", "SEARCH", "Search Flights", "Search flights", "Find flights", "Book"]:
            try:
                btn = page.get_by_role("button", name=re.compile(rf"^{re.escape(label)}$", re.IGNORECASE))
                if await btn.count() > 0:
                    await btn.first.click(timeout=5000)
                    logger.info("AIX: clicked search")
                    return
            except Exception:
                continue
        try:
    ...
    def _parse_single_flight(self, flight: dict, req: FlightSearchRequest, booking_url: str) -> Optional[FlightOffer]:
        ...
        return FlightOffer(
            id=f"ix_{hashlib.md5(str(flight_key).encode()).hexdigest()[:12]}",
    ...
    def _build_segment(self, seg: dict, default_origin: str, default_dest: str) -> FlightSegment:
        ...
        carrier = seg.get("carrierCode") or seg.get("carrier") or seg.get("airline") or "IX"
        return FlightSegment(
            airline=carrier, airline_name="Air India Express", flight_no=flight_no,
    ...
    def _build_response(self, offers: list[FlightOffer], req: FlightSearchRequest, elapsed: float) -> FlightSearchResponse:
        ...
        logger.info("AIX %s→%s returned %d offers in %.1fs (Playwright)", req.origin, req.destination, len(offers), elapsed)
        h = hashlib.md5(f"aix{req.origin}{req.destination}{req.date_from}".encode()).hexdigest()[:12]
    ...
    def _build_booking_url(req: FlightSearchRequest) -> str:
        ...
        return (
            f"https://www.airindiaexpress.com/booking?origin={req.origin}"
