"""
Spring Airlines Playwright connector — browser automation to bypass WAF.

Spring Airlines (IATA: 9C) is a Chinese LCC headquartered in Shanghai.
Website: global.ch.com/en or flights.ch.com/en (international site).

Strategy:
1. Navigate to global.ch.com/en homepage
2. Dismiss cookie/popup banner
...
_LOCALES = ["en-US", "en-GB", "zh-CN", "en-CN"]
_TIMEZONES = [
...
async def _get_browser():
    ...
    async with lock:
        ...
        logger.info("Spring: Playwright browser launched (headed Chrome)")
        return _browser


class SpringConnectorClient:
    """Spring Airlines Playwright connector — homepage form search + API interception."""
    ...
    async def search_flights(self, req: FlightSearchRequest) -> FlightSearchResponse:
        ...
        try:
            ...
            logger.info("Spring: loading homepage for %s→%s", req.origin, req.destination)
            # Try international site first, fall back to flights.ch.com
            loaded = False
            for url in ["https://global.ch.com/en", "https://flights.ch.com/en", "https://www.ch.com/en"]:
                try:
            ...
            if not ok:
                logger.warning("Spring: origin fill failed")
                return self._empty(req)
            await asyncio.sleep(0.5)

            ok = await self._fill_airport_field(page, req.destination, 1)
            if not ok:
                logger.warning("Spring: destination fill failed")
                return self._empty(req)
            await asyncio.sleep(0.5)

            ok = await self._fill_date(page, req)
            if not ok:
                logger.warning("Spring: date fill failed")
                return self._empty(req)
            ...
            try:
                ...
                logger.warning("Spring: timed out waiting for API response")
            ...
            logger.error("Spring Playwright error: %s", e)
            return self._empty(req)
        finally:
            await context.close()
    ...
    async def _dismiss_cookies(self, page) -> None:
        for label in [
            ...
            try:
                ...
                continue
        try:
            close_btn = page.locator("[class*='close'], .popup .close, .modal .close, [aria-label='Close']").first
    ...
    async def _set_one_way(self, page) -> None:
        for label in ["One Way", "One-way", "One way", "单程"]:
            try:
    ...
    async def _fill_airport_field(self, page, iata: str, index: int) -> bool:
        origin_labels = ["From", "Origin", "Departure", "出发"]
        dest_labels = ["To", "Destination", "Arrival", "到达"]
        ...
        try:
            ...
            logger.debug("Spring: field %d error: %s", index, e)
        try:
    ...
    async def _fill_date(self, page, req: FlightSearchRequest) -> bool:
        ...
        try:
            for name in ["Departure", "Depart", "Date", "出发日期"]:
            ...
            logger.warning("Spring: date error: %s", e)
            return False

    async def _click_search(self, page) -> None:
        for label in ["Search", "SEARCH", "Search Flights", "Find Flights", "搜索航班"]:
            try:
                btn = page.get_by_role("button", name=re.compile(rf"^{re.escape(label)}$", re.IGNORECASE))
                if await btn.count() > 0:
    ...
    def _parse_single_flight(self, flight: dict, req: FlightSearchRequest, booking_url: str) -> Optional[FlightOffer]:
        ...
        return FlightOffer(
            id=f"9c_{hashlib.md5(str(flight_key).encode()).hexdigest()[:12]}",
            price=round(best_price, 2), currency=req.currency,
            price_formatted=f"{best_price:.2f} {req.currency}",
            outbound=route, inbound=None, airlines=["Spring Airlines"], owner_airline="9C",
            booking_url=booking_url, is_locked=False, source="spring_direct", source_tier="free",
        )
    ...
    def _build_segment(self, seg: dict, default_origin: str, default_dest: str) -> FlightSegment:
        ...
        carrier = seg.get("carrierCode") or seg.get("carrier") or seg.get("airline") or "9C"
        return FlightSegment(
            airline=carrier, airline_name="Spring Airlines", flight_no=flight_no,
    ...
    def _build_response(self, offers: list[FlightOffer], req: FlightSearchRequest, elapsed: float) -> FlightSearchResponse:
        ...
        logger.info("Spring %s→%s returned %d offers in %.1fs", req.origin, req.destination, len(offers), elapsed)
        h = hashlib.md5(f"spring{req.origin}{req.destination}{req.date_from}".encode()).hexdigest()[:12]
    ...
    def _build_booking_url(req: FlightSearchRequest) -> str:
        ...
        return (
            f"https://global.ch.com/en/booking/search?origin={req.origin}"
